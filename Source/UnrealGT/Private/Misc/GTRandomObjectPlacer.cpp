...
#include "GTRandomObjectPlacer.h"
...
AGTRandomObjectPlacer::AGTRandomObjectPlacer()
    : Super(), BoundingBox(200.f, 400.f),
      GeneratorTriesBeforePointRejection(50), MaximumHeightDifference(10000.f),
      bPlaceMeshActors(true) {
  RootComponent =
      CreateDefaultSubobject<USceneComponent>(TEXT("DefaultSceneRoot"));

  PreviewBoxComponent =
      CreateDefaultSubobject<UBoxComponent>(TEXT("PreviewBox"));
...
void AGTRandomObjectPlacer::PostEditChangeProperty(
...
bool AGTRandomObjectPlacer::ShouldTickIfViewportsOnly() const { return true; }

#endif

void AGTRandomObjectPlacer::BeginPlay() { Super::BeginPlay(); }
...
void AGTRandomObjectPlacer::GeneratePoints() {
  ...
  GridSize =
      FIntPoint(BoundingBox.X / GridCellSize, BoundingBox.Y / GridCellSize);
  ...
  SpawnPointsToSpawnInfoID.Add(SpawnPoints[FirstSpawnIndex],
                               GetWeightedRandomSpawnInfoID());
  ...
  while (SpawnPoints.Num() > 0) {
    ...
    for (int I = 0; I < GeneratorTriesBeforePointRejection; I++) {
      ...
      float MinDistanceBetweenObjects =
          CurrentSpawnInfo.Radius + NewObjectSpawnInfo.Radius;

      FVector2D NewPoint =
          CurrentSpawnPoint +
          Dir * FMath::RandRange(MinDistanceBetweenObjects,
                                 MinDistanceBetweenObjects * 2.f);
  ...
  for (FGTRandomObjectSpawnInfo &ObjectSpawnInfo : ObjectSpawnInfos) {
    if (ObjectSpawnInfo.Mesh) {
      ObjectSpawnInfo.RandomMaterials.AddUnique(
          ObjectSpawnInfo.Mesh->GetMaterial(
              ObjectSpawnInfo.RandomMaterialSlotID));
    }
  }
...
void AGTRandomObjectPlacer::SpawnInstanceMeshComponents() {
  ...
  for (FGTRandomObjectSpawnInfo &ObjectSpawnInfo : ObjectSpawnInfos) {
    if (ObjectSpawnInfo.Mesh) {
      for (UMaterialInterface *MaterialOption :
           ObjectSpawnInfo.RandomMaterials) {
      ...
      if (OutHit.bBlockingHit) {
        ...
        if (bHitAllowedActor) {
          ...
          bool bSphereTraceHit = GetWorld()->SweepMultiByChannel(
              ...
              OutHit.Location, FQuat::Identity, ECC_Visibility,
          ...
          if (bSphereTraceHit) {
            ...
            for (const FHitResult &SphereHit : OutHits) {
              if (SphereHit.Actor.IsValid()) {
                if (!bAllowPlaceOnLandscape &&
                    Cast<ALandscape>(SphereHit.Actor)) {
        ...
        if (bShouldPlaceActor) {
          ...
          if (ObjectSpawnInfo.bShouldAlignToGround) {
            GroundRotator = FMatrix(FVector::CrossProduct(GetActorRightVector(),
                                                          OutHit.ImpactNormal),
                                    GetActorRightVector(), OutHit.ImpactNormal,
                                    FVector::ZeroVector)
                                .Rotator();
          }
          ...
          if (bUseInstancedStaticMesh) {
            for (UActorComponent *InstancedComponent :
            ...
            MeshComponent->SetMaterial(ObjectSpawnInfo.RandomMaterialSlotID,
                                       RandomMaterial);
...
bool AGTRandomObjectPlacer::IsValidNewPoint(const FVector2D &Point,
                                            float Radius) {
  ...
  for (int X = GridCenter.X - CellsToCheck; X <= GridCenter.X + CellsToCheck;
       X++) {
    for (int Y = GridCenter.Y - CellsToCheck; Y <= GridCenter.Y + CellsToCheck;
         Y++) {
      if (X < 0 || Y < 0) {
        return false;
      }
...
void AGTRandomObjectPlacer::BlockGrid(const FVector2D &Center, float Radius) {
  ...
  for (int X = GridCenter.X - CellsToBlock; X <= GridCenter.X + CellsToBlock;
       X++) {
    for (int Y = GridCenter.Y - CellsToBlock; Y <= GridCenter.Y + CellsToBlock;
         Y++) {

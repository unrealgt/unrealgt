...
#include "Misc/GTCameraMovementComponent.h"
    ...
    FActorComponentTickFunction* ThisTickFunction)
{
    ...
    else if (bFollowRail)
    {
        ...
        {
            float Progress =
                (RailSpeed / FollowRail->GetRailSplineComponent()->GetSplineLength()) * DeltaTime;

            float NewPosition = FMath::Min(FollowRail->CurrentPositionOnRail + Progress, 1.f);
            ...
            {
                FQuat const RailRot =
                    FollowRail->GetRailSplineComponent()->GetQuaternionAtDistanceAlongSpline(
                        FollowRail->GetRailSplineComponent()->GetSplineLength() * NewPosition,
                        ESplineCoordinateSpace::World);
                GetOwner()->SetActorRotation(
                    FMath::QInterpTo(GetOwner()->GetActorQuat(), RailRot, DeltaTime, 1.5f));
            }

            FollowRail->CurrentPositionOnRail = NewPosition;

            if (bDestroyOwnerAtEnd && FMath::IsNearlyEqual(FollowRail->CurrentPositionOnRail, 1.f))
            {
                GetOwner()->Destroy();
            }
        }

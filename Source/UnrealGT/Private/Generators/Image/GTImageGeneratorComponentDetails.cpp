...
TSharedRef<IDetailCustomization> FGTImageGeneratorComponentDetails::MakeInstance()
{
    return MakeShareable(new FGTImageGeneratorComponentDetails());
}

void FGTImageGeneratorComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
    ...
    Cat.AddProperty(
        GET_MEMBER_NAME_CHECKED(UGTCameraCalibrationComponent, CameraOneReference),
        UGTCameraCalibrationComponent::StaticClass());
    Cat.AddProperty(
        GET_MEMBER_NAME_CHECKED(UGTCameraCalibrationComponent, CameraTwoReference),
        UGTCameraCalibrationComponent::StaticClass());

    Cat.AddCustomRow(NSLOCTEXT(
        ...
        .HAlign(HAlign_Left)
            [SNew(SBox).MaxDesiredWidth(300.f)
                 [SNew(SUniformGridPanel).SlotPadding(2.0f) +
                  SUniformGridPanel::Slot(0, 0)[SNew(STextBlock)
                                                    .Text(NSLOCTEXT(
                                                        "CameraCalibrationComponentDetails",
                  ...
                  SUniformGridPanel::Slot(0, 1)
                      [SNew(SButton)
                           .OnClicked(
                               this,
                               &FGTImageGeneratorComponentDetails::GenerateMonoCameraParameters)
                  ...
                  SUniformGridPanel::Slot(1, 1)
                      [SNew(SButton)
                           .OnClicked(
                               this,
                               &FGTImageGeneratorComponentDetails::GenerateStereoCameraParameters)
                           .ToolTipText(NSLOCTEXT(
                               "CameraCalibrationComponentDetails",
...
FReply FGTImageGeneratorComponentDetails::GenerateStereoCameraParameters()
{
    ...
    FString CameraParameterString = FString::Format(
        ...
        {FocalLengthOne.X,
         ...
         ImageSize.X,
         ImageSize.Y,
         FRotationMatrix(Rotation).ToString(),
         Translation.X,
         Translation.Y,
         Translation.Z});
    FPlatformApplicationMisc::ClipboardCopy(*CameraParameterString);
...
UGTImageGeneratorBase* FGTImageGeneratorComponentDetails::GetImageGeneratorFromReference(

...
#include "GTDepthImageGeneratorComponent.h"
...
{
    if (SceneCaptureComponent && SceneCaptureComponent->TextureTarget && SceneCaptureComponent->TextureTarget->IsValidLowLevel())
    {
        ...
        FCanvasTileItem TileItem(FVector2D::ZeroVector,
        ...
        FTextureRenderTargetResource* RenderTargetResource = DebugTextureTarget->GameThread_GetRenderTargetResource();
        ...
        for (int PixelID = 0; PixelID < DepthImage.Pixels.Num(); PixelID++)
        {
            ...
            float PixelDepth = ((PixelColor.R + PixelColor.G * 256.f + PixelColor.B * 256.f * 256.f) / (MaxZ * 10.f)) * 1000.f;
            float PixelDepthLogScale = FMath::Clamp(FMath::Loge(PixelDepth) / 9.72777705446f, 0.f, 1.f) * 255;
            DepthImage.Pixels[PixelID] = FColor(PixelDepthLogScale, PixelDepthLogScale, PixelDepthLogScale, 255);
        }

        UTexture2D* ConvertedDepthTexture = GetSceneCaptureComponent()->TextureFromImage(DepthImage);
        FTexture* ConvertedDepthTextureTextureResource = ConvertedDepthTexture->Resource;
        FCanvasTileItem ConvertedDepthTileItem(FVector2D(TileItem.Size.X, 0.f),
        ...
        ConvertedDepthTileItem.BlendMode = FCanvas::BlendToSimpleElementBlend(EBlendMode::BLEND_Opaque);

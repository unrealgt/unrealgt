...
#include "GTFileStreamerComponent.h"
...
public:
  FGTSaveFileTask(const FString &TotalFileName, const TArray<uint8> &Data)
      : TotalFileName(TotalFileName), Data(Data) {}

protected:
  FString TotalFileName;
  TArray<uint8> Data;

  void DoWork() { FFileHelper::SaveArrayToFile(Data, *TotalFileName); }

  FORCEINLINE TStatId GetStatId() const {
    RETURN_QUICK_DECLARE_CYCLE_STAT(FGTSaveFileTask,
...
UGTFileStreamerComponent::UGTFileStreamerComponent()
    : FileNameFormat(TEXT("{ID}_{Time}.txt")), IDCounter(0) {}

void UGTFileStreamerComponent::BeginPlay() { Super::BeginPlay(); }

void UGTFileStreamerComponent::OnDataReady(const TArray<uint8> &Data,
  ...
  FString SessionDir = FPaths::ConvertRelativePathToFull(FPaths::Combine(
      FPaths::ProjectSavedDir(), TEXT("UnrealGT"), GetWorld()->GetMapName(),
      SessionStartTime.ToString(TimeFormat), GetName()));
  ...
  (new FAutoDeleteAsyncTask<FGTSaveFileTask>(TotalFileName, Data))
      ->StartBackgroundTask();
}

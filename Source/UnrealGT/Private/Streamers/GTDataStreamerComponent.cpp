...
void UGTDataStreamerComponent::BeginPlay() {
  ...
  UGTDataGeneratorComponent *Generator =
      GeneratorReference.GetComponent(GetOwner());

  if (Generator) {
    Generator->DataReadyDelegate.AddDynamic(
        this, &UGTDataStreamerComponent::OnDataReady);
  }
}

void UGTDataStreamerComponent::OnDataReady(const TArray<uint8> &Data,

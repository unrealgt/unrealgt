...
void SGTDebugWidget::Construct(const FArguments &InArgs) {
  ...
  Viewport =
      MakeShareable(new FSceneViewport(ViewportClient.Get(), ViewportWidget));
...
void SGTDebugWidget::SetDataGenerator(
    UGTDataGeneratorComponent *DataGenerator) {
  ((FGTDebugViewportClient *)Viewport->GetClient())
  ...
  if (World) {
    for (TObjectIterator<UGTDataGeneratorComponent> Itr; Itr; ++Itr) {
      if (Itr->GetWorld() == World &&
          Itr->GetName().Contains(NewText.ToString())) {
...
void SGTDebugWidget::Tick(const FGeometry &AllottedGeometry,

...
#include <Private/SSingleProperty.h>
...
void FGTGeneratorReferenceCustomization::CustomizeChildren(
  ...
  if (!IsSelectionValid()) {
    FNotificationInfo Info(
        FText::Format(LOCTEXT("InvalidComponentReference",
                              "Component Reference {0} is no longer valid."),
    ...
    GEditor->PlayEditorSound(TEXT("/Engine/EditorSounds/Notifications/"
...
TArray<FName> FGTGeneratorReferenceCustomization::GetAvailableComponentNames() {
  ...
  for (TFieldIterator<UObjectPropertyBase> Prop(OwnerClass); Prop; ++Prop) {
    if (Prop->PropertyClass->IsChildOf(
...
FString
FGTGeneratorReferenceCustomization::GetSelectedComponentNameForComboBox() {
...
void FGTGeneratorReferenceCustomization::OnComponentNameSelected(
    const FString &SelectedName) {
  StructPropertyHandle->GetChildHandle(TEXT("ComponentProperty"))
      ->SetValue(FName(*SelectedName));
}
